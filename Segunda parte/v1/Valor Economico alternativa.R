...
#En este ejercicio se supone que el activo esta compuesto por prestamos a tasa repactable de 9% 
#n=5 y un  Plazo fijo a tasa fija con i= 10%

#Prestamo a tasa variable (ACTIVO)
t=1:5
tasa_A=0.09
...
#Plazo fijo Anual tasa fija, tasa=10% (Pasivo)
tasa_P=0.10
...
#Si sube~Escenario suba 200 bps = 0.02
bps=200/10000
tasa_A_s=tasa_A+bps

#Prestamo a tasa variable (ACTIVO)
(CF_Activo_s=balance$Contable[1]*c(tasa_A,rep(tasa_A_s,3),(1+tasa_A_s))) #Se modifica son variables
...
#Plazo fijo Anual fija, tasa=9% (Pasivo)
tasa_P_s=tasa_P+bps
(CF_Pasivo_s=balance$Contable[2]*c(rep(tasa_P,4),(1+tasa_P))) 
...
#Contablemente es el mismo, pero el valor economico aumenta. 
