...
W_act1 <- Tabla$Cantidades[1]*Tabla$Precio[1]
W_act2 <- Tabla$Cantidades[2]*Tabla$Precio[2]
W_act3 <- Tabla$Cantidades[3]*Tabla$Precio[3]

W <- W_act1+W_act2+W_act3

w<- c(W_act1, W_act2, W_act3)/W
w

#Calculo el VaR Normal (parametrico) Individual y Cartera
VaR_1_indiv <- Zc*Tabla$Volatilidad[1]*sqrt(h)*W_act1
VaR_1_indiv

VaR_2_indiv <- Zc*Tabla$Volatilidad[2]*sqrt(h)*W_act2
VaR_2_indiv

VaR_3_indiv <- Zc*Tabla$Volatilidad[3]*sqrt(h)*W_act3
VaR_3_indiv

VaR_P_Indiv_nd <- VaR_1_indiv+VaR_2_indiv+VaR_3_indiv
VaR_P_Indiv_nd

#VaR Diversificado
matriz_correlaciones = matrix(c(1, rho_1_2,rho_1_3,
                                rho_1_2,1,rho_2_3,
                                rho_1_3,rho_2_3,1), nrow=3)
...
rendimientos <- data.frame(RA=Rend_AAPL,RI=Rend_IBM, RG=Rend_GOOG)
...
ggplot(Tabla, aes(x = date)) + 
  ...
  scale_color_manual(values = c("AAPL" = "lightblue", "IBM" = "lightyellow", "GOOG"= "lightgreen")) +
...
W_act1_ = 70 * Tabla$AAPL[nrow(Tabla)] #de mi variable stoc data, de la columna de apple mi ultima fila (el ultimo precio) nrow --> ultima fila
W_act2_ = 100 * Tabla$IBM[nrow(Tabla)] #Valor de la posicion en IBM
W_act3_ = 50* Tabla$GOOG[nrow(Tabla)]

W_ = W_act1_ + W_act2_ + W_act3_ #Total de mi posicion
w_ <- c(W_act1_, W_act2_, W_act3_)/W_ #Proporcion invertida 
...
(VaR_AAPL_Normal = z*volatilidad_AAPL*sqrt(h)*W_act1_) #VaR si solo tuviera mis 700 acciones de apple #normal = diversificado
z*sd(Rend_AAPL)*sqrt(5)*W_act1_
(VaR_AAPL_Normal/W_act1_ )#Son un 6.7% de la cartera. Hay un 1% de probabiidad de que la perdida en un horizonte de 5 dias, sea superior a 892 dolares, si solo tengo a apple en mi cartera 

(VaR_IBM_Normal = z*volatilidad_IBM*sqrt(h)*W_act2_)
z*sd(Rend_IBM)*sqrt(5)*W_act2_
(VaR_IBM_Normal/W_act2_) #Si solo tuviera iBM en el portafolio, en un horizonte de 5 dias, hay un 1% de probabilidad de que la perdida sea mayor a 766 dolares. Representa un 5.7% del portafolio

(VaR_GOOG_Normal = z*volatilidad_GOOG*sqrt(h)*W_act3_)
(VaR_GOOG_Normal/W_act3_)
